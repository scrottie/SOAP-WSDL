...
sub wsdlinit {
  ...
  foreach my $ns( @{ $nameSpaces } ) {
    ...
    if($ns->getData eq "http://schemas.xmlsoap.org/wsdl/"){
      $self->_wsdl_wsdlExplicitNS ($ns->getPrefix?$ns->getPrefix.":":"");
    }

    # the schema namespace is hardcoded in to the SOAP::Constants package,
    # in the Lite.pm module
    if (defined $SOAP::Constants::XML_SCHEMAS{$ns->getData } and
	$SOAP::Constants::XML_SCHEMAS{$ns->getData }=~/SOAP::XMLSchema\d+/){
  ...
  $self->_wsdl_soapns($self->_wsdl_ns->{'http://schemas.xmlsoap.org/wsdl/soap/'});
  
  #the default namespaces for types
  $self->{_WSDL}->{_type_ns} = "";
  $self->_wsdl_schemans and
    $self->{_WSDL}->{_type_ns}  .= $self->_wsdl_schemans.":|";
  
  #TBD: the apache soap special case has to be handled elsewhere
  $nsHash->{'http://xml.apache.org/xml-soap'} and
    $self->{_WSDL}->{_type_ns} .= $nsHash->{'http://xml.apache.org/xml-soap'} . "|";;
...
sub call {
  ...
  $binding = $port->findvalue('@binding')
    || die "Error processing WSDL: Cannot find the binding for the service $path";
  ...
  $location = $address->findvalue('@location')->value 
    || die "Error processing WSDL: Cannot find the port for the location in service $path";
  $self->proxy($location);
  ...
  $data{"wsdl_${mode}_name"} and
    $path .= "[\@name='".$data{"wsdl_${mode}_name"}."']";

  #now we can get the soapaction
  my $soapActionPath = "$path/../".$self->_wsdl_soapns."operation/\@soapAction";
  ...
  $encodingStyle and $self->wsdl_encoding($self->_wsdl_ns->{$encodingStyle});
  ...
  my $methodEncoded=SOAP::Data->name( $inputMessageName )->attr( { "xmlns" => $callNamespace } );
  unless ($self->{_WSDL}->{no_dispatch}) { 
    return $self->SUPER::call($methodEncoded => @param,@{$data{soap_headers}});	
  } else {
...
sub _load_method{
  ...
  *$method = sub{
    my $self = shift;
    return (@_) ? $self->{_WSDL}->{$param} = shift : $self->{_WSDL}->{$param}?$self->{_WSDL}->{$param}:""};
}
...
sub _make_finder(){
  ...
  *$method = sub {
	...
	$self->{_WSDL}->{ cache }->{ $path }= $data if ($self->{_WSDL}->{ caching });
      }
      if(!$data){
	$dieIfError and print("Error processing WSDL: can't find the path '$path'\n"),exit;
      }
      return $data;
    };
}
...
sub encode {
  ...
  if ( ($p->{type}) ) {
	...
	."${schemaNS}simpleType[\@name='$typeName']/"
      ...
      $simpleType and my $baseType = $simpleType->findvalue('@base')->value;
      ...
      if(defined ($baseType) and $baseType eq $wsdl_encoding."Array"){
	...
	my $simpleType = $self->{_WSDL}->{xpath}->find($path)->shift;
	$simpleType and $baseType = $simpleType->findvalue('@'.($self->_wsdl_wsdlExplicitNS).'arrayType')->value;
    ...
    if($1 ne $schemaNS){
      ...
      my $nodeSet = $self->_wsdl_find("$path/preceding-sibling::${schemaNS}import");
